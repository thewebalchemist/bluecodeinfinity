...
const techDirectory = path.join(process.cwd(), 'technologies')

export function getSortedTech() {
  // Get file names under /tech
    const fileNames = fs.readdirSync(techDirectory)
    const tech = fileNames.map(fileName => {
    ...
    // Sort tech by date
    return tech.sort(({ date: a }, { date: b }) => {
        if (a < b) {
        return 1
        } else if (a > b) {
        return -1
